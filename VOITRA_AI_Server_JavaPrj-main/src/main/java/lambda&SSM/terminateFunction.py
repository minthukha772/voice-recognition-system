...
def backup_dir(ASGNAME):
    ...
    if auto_scaling_group == 'EC2ContainerService-Cluster-EcsInstanceAsg-1KS7UP320VVS7':
...
def abandon_lifecycle(life_cycle_hook, auto_scaling_group, instance_id):
    ...
    try:
        ...
        logger.error(
            "Lifecycle hook abandon could not be executed: %s", str(e))
        return None
        ...
        if LIFECYCLE_KEY in message and ASG_KEY in message:
            ...
            if check_document():
                command_id = send_command(
                    instance_id, life_cycle_hook, auto_scaling_group)
                print(command_id)
                if command_id != None:
                    if check_command(command_id, instance_id):
                        ...
                        abandon_lifecycle(
                            life_cycle_hook, auto_scaling_group, instance_id)
                else:
                    abandon_lifecycle(
                        life_cycle_hook, auto_scaling_group, instance_id)
            else:
